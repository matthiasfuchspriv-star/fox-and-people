...
export function LoginShell({ children }: { children: ReactNode }) {
  return (
    <main className="min-h-screen grid lg:grid-cols-[1.1fr_1fr]">
      <section className="hidden lg:flex flex-col justify-between p-12 text-white relative overflow-hidden" style={{ background: "#10222a" }}>
        ...
        <div className="relative max-w-md">
          <p className="section-title mb-3">Personaldisposition · Verrechnung · Controlling</p>
          <h1 className="font-display text-[42px] leading-[1.05] font-bold tracking-[-0.03em]">Ein Arbeitsplatz für den ganzen Tag in der Personaldisposition.</h1>
...
export function Logo({ dark, size = 36 }: { dark?: boolean; size?: number }) {
  ...
  return (
    <span className="inline-flex items-center" style={{ height: size }}>
      <span className="wordmark" style={{ fontSize: size * 0.52, color: dark ? "#10222a" : "#f7f6f2", lineHeight: 1 }}>Fox <span style={{ color: "#b4522c" }}>&</span> People</span>
    </span>
  );
}
