        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            <Card title={<span>Mitarbeiter bewerten den Kunden {avgVonMa && <span className="ml-2"><Fuechse n={avgVonMa} zahl /></span>}</span>} className="reveal">
              {k.kundenBewertungen.length ? <ul className="divide-y divide-line">{k.kundenBewertungen.map((b) => <li key={b.id} className="py-3 flex gap-3"><Fuechse n={b.sterne} /><div className="flex-1 min-w-0"><Link href={`/personen/${b.personId}`} className="font-semibold text-[14px] hover:text-brand">{b.person.vorname} {b.person.nachname}</Link><span className="text-muted text-[12.5px]"> · {datum(b.datum)} · {b.quelle === "PORTAL" ? "über Portal" : "intern erfasst"}</span><MerkmalChips merkmale={b.merkmale} />{b.kommentar && <p className="text-[12.5px] mt-0.5">{b.kommentar}</p>}</div>{b.wiederArbeiten ? <Badge tone="teal">würde wieder</Badge> : <Badge tone="red">nicht wieder</Badge>}</li>)}</ul> : <p className="text-muted text-[12.5px]">Noch keine Rückmeldungen von Mitarbeitern. Mitarbeiter können den Kunden auch selbst im Portal bewerten.</p>}
            </Card>
            <Card title="Rückmeldung eines Mitarbeiters erfassen" className="reveal reveal-2">
              <form action={kundenBewertungAnlegen.bind(null, id)} className="space-y-3">
                ...
                <label className="flex items-center gap-2 text-[14px]"><input type="checkbox" name="wieder" defaultChecked /> Würde dort wieder arbeiten</label>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <Card title="Ansprechpartner hinzufügen" className="reveal reveal-2">
            <form id="ap-neu" action={ansprechpartnerAnlegen.bind(null, id)} className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-[14px]"><input type="checkbox" name="istHaupt" /> Hauptansprechpartner</label>
              <Field label="Rollen" help="Steuert, wer Rechnungen, Stundennachweise, Dispo-Mails und Bewertungsanfragen bekommt."><div className="space-y-1 mt-1">{AP_ROLLEN.map(([r, l]) => <label key={r} className="flex items-center gap-2 text-[12.5px]"><input type="checkbox" name="rollen" value={r} /> {l}</label>)}</div></Field>
              <button className="btn btn-primary w-full justify-center">Speichern</button>
            </form>
          </Card>
        </div>
      ...
      {tab === "konditionen" && (
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <Card title="Verrechnungssätze lt. angenommenen Angeboten" pad={false} className="reveal reveal-2">
            {k.konditionen.length ? <table className="table"><thead><tr><th>Rolle</th><th className="r">Satz €/Std</th><th className="r">Ü50 %</th><th className="r">Ü100 %</th></tr></thead><tbody>{k.konditionen.map((x) => <tr key={x.id}><td className="font-semibold">{x.rolle}</td><td className="r num">{eur(x.stundensatz)}</td><td className="r num">+{Math.round(x.ueberstundenZuschlag * 100)} %</td><td className="r num">+{Math.round(x.wochenendZuschlag * 100)} %</td></tr>)}</tbody></table> : <p className="p-5 text-[12.5px] text-muted">Noch kein angenommenes Angebot – Sätze erscheinen automatisch, sobald ein Angebot auf „Angenommen“ steht.</p>}
      ...
      {tab === "dokumente" && (
        <div className="grid lg:grid-cols-3 gap-4">
          <Card title="Dokumente" pad={false} className="lg:col-span-2 reveal">
            ...
            {k.vertraege.length > 0 && <div className="px-5 py-3 border-t border-line text-[12.5px]">Verträge: {k.vertraege.map((v) => <Link key={v.id} href={`/vertraege/${v.id}`} className="text-brand font-semibold mr-3">{v.nummer}</Link>)}</div>}
          </Card>
      ...
      {tab === "historie" && (
        <div className="grid lg:grid-cols-3 gap-4">
          <Card title="Kunden-Historie" className="lg:col-span-2 reveal">
            {k.aktivitaeten.length ? <ol className="relative border-l border-line ml-2">{k.aktivitaeten.map((a) => <li key={a.id} className="ml-5 pb-5 last:pb-0"><span className="absolute -left-[5px] mt-1.5 w-2.5 h-2.5 rounded-full bg-brand border-2 border-surface" /><div className="text-[12.5px] text-muted">{datum(a.zeitpunkt)} · {a.nutzerName} · {a.typ}</div><div className="text-[14px]">{a.text}</div></li>)}</ol> : <Empty title="Noch keine Einträge" />}
          </Card>
