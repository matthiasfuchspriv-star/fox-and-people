...
export function MailForm({ vorlagen, start, signatur }: { vorlagen: { key: string; label: string; betreff: string; text: string }[]; start: string; signatur: string }) {
  ...
  return (
    <>
      ...
      <div className="field"><label className="label">Text <span className="text-red">*</span></label><textarea name="text" required value={text} onChange={(e) => setText(e.target.value)} rows={14} className="textarea" /><div className="mt-2 text-[12px] text-muted whitespace-pre-line border-l-2 border-line pl-3">{signatur}</div></div>
    </>
  );
}
