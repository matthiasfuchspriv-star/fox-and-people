  ...
  return (
    <AppShell navLabels={navLabels(t)}>
      ...
      <div className="px-4 pt-4 space-y-4">
            ...
            <ul className="divide-y divide-line">{docs.map((d) => (
              <li key={d.id}>
                <a href={`/dokumente/${d.id}`} target="_blank" className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0"><div className="font-semibold text-[13.5px] truncate">{d.dateiname}</div><div className="text-[12px] text-muted">{datum(d.hochgeladenAm)}</div></div>
                </a>
              </li>
            ))}</ul>
          ) : <p className="text-muted text-[13.5px] px-5 pb-4">{t("lohn.keineLohnzettel")}</p>}
        </section>
        ...
        {zk && zk.sollGesamt > 0 && (
          <section className="card card-pad">
            ...
            <div className="flex items-end gap-2">
              <div className={`font-display font-extrabold text-[24px] leading-none ${zk.saldo < 0 ? "text-red" : "text-teal"}`}>{zk.saldo > 0 ? "+" : ""}{zk.saldo.toLocaleString("de-AT", { maximumFractionDigits: 1 })} h</div>
        ...
        <p className="text-[12px] text-muted">{t("lohn.fragen")}</p>
      </div>
    </AppShell>
  );
}
