    ...
    <AppShell>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <section className="card card-pad flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-surface-2 overflow-hidden flex items-center justify-center font-display font-bold text-[22px] shrink-0">{p.fotoDokumentId ? <img src={`/dokumente/${p.fotoDokumentId}`} alt="" className="w-full h-full object-cover" /> : `${p.vorname[0]}${p.nachname[0]}`}</div>
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[12.5px] text-muted mt-0.5">Profil zu {vollst} % vollständig</div>
          </div>
        </section>

        <section className="card card-pad space-y-3">
          <div className="section-title">Benachrichtigungen</div>
          <p className="text-[12.5px] text-muted">Wir sagen dir Bescheid, wenn deine Stunden bestätigt sind, eine Nachricht da ist, dein Urlaub genehmigt wurde oder ein neuer Lohnzettel bereitliegt. Kein Werbe-Spam.</p>
          <PushKnopf />
        </section>
        ...
        {!p.datenschutzAkzeptiertAm && (
          <form action={datenschutzBestaetigen} className="card card-pad space-y-3 border-2 border-fox">
            ...
            <div className="text-[12.5px] whitespace-pre-line max-h-56 overflow-y-auto rounded-md bg-surface-2 p-3">{DATENSCHUTZ_TEXT}</div>
            <label className="flex items-start gap-2 text-[14px]"><input type="checkbox" name="ok" required className="mt-0.5" /> Ich habe die Datenschutzinformation gelesen und bin einverstanden.</label>
            <label className="flex items-start gap-2 text-[14px]"><input type="checkbox" name="foto" className="mt-0.5" /> Mein Foto darf im Profil verwendet werden, das Fox &amp; People an Kunden schickt (freiwillig, jederzeit widerrufbar).</label>
        ...
        <form action={profilSpeichern} className="card card-pad space-y-3">
          ...
          <label className="flex items-center gap-2 text-[14px]"><input type="checkbox" name="fuehrerschein" defaultChecked={p.fuehrerschein} /> Führerschein & Auto vorhanden</label>
        ...
        <section className="card card-pad text-[12.5px] space-y-1">
        ...
        <p className="text-[12.5px] text-muted pb-4">Datenschutz: Deine Daten werden ausschließlich für das Dienstverhältnis verwendet (Datenschutzinformation für Mitarbeiter). Löschung oder Auskunft: office@foxandpeople.at</p>
      </div>
    </AppShell>
  );
}
