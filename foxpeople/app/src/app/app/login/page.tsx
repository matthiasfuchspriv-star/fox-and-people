...
export default async function AppLogin({ searchParams }: { searchParams: Promise<{ schritt?: string; e?: string; an?: string; test?: string; fehler?: string }> }) {
  ...
  return (
    <main className="min-h-screen flex flex-col bg-brand-ink">
      <div className="px-6 pt-14 pb-8 text-white"><Logo size={40} /><div className="text-white/60 text-[12.5px] mt-2">Mitarbeiter-App</div></div>
      <div className="flex-1 bg-bg rounded-t-[22px] px-6 pt-7 pb-10 shadow-[0_-12px_40px_rgba(0,0,0,.25)]">
        {!codeSchritt ? (
          <form action={codeAnfordern} className="space-y-4">
            ...
            <input name="eingabe" required autoFocus inputMode="email" autoComplete="username" placeholder="E-Mail-Adresse" className="input !py-3 !text-[17px] !rounded-[11px]" defaultValue={sp.e ?? ""} />
            <button className="btn btn-primary btn-lg w-full">Code anfordern</button>
            <p className="text-[12.5px] text-muted">Keine E-Mail hinterlegt oder Adresse geändert? Ruf uns an: +43 676 4574096</p>
          </form>
        ) : (
          <form action={codePruefen} className="space-y-4">
            ...
            {sp.test && <div className="alert alert-amber"><span>Testmodus (kein SMTP): dein Code ist <b className="num text-[17px]">{sp.test}</b></span></div>}
            ...
            <input name="code" required autoFocus inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="one-time-code" placeholder="123456" className="input !py-3 !text-[28px] !rounded-[11px] text-center tracking-[0.4em] num" />
            <button className="btn btn-primary btn-lg w-full">Anmelden</button>
            <a href="/app/login" className="block text-center text-[12.5px] text-brand font-semibold">Neuen Code anfordern</a>
          </form>
        )}
      </div>
    </main>
