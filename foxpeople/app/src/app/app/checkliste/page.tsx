    ...
    <AppShell>
      ...
      <div className="px-4 pt-4 space-y-4">
        <section className="card card-pad">
          ...
          <div className="flex items-end gap-2"><div className="font-display font-extrabold text-[26px] leading-none">{fertig} von {punkte.length}</div><div className="text-[12.5px] text-muted pb-1">erledigt</div></div>
        ...
        <section className="card">
          ...
          {offen.length ? (
            <ul className="divide-y divide-line">{offen.map((o) => { const x = SELBST[o.key]; return (
              <li key={o.key}>
                {x.href ? <Link href={x.href} className="flex items-start gap-3 px-5 py-3 text-[13.5px]"><Circle size={17} className="text-amber shrink-0 mt-0.5" /><span className="flex-1">{x.text}{o.hinweis && <span className="block text-[12px] text-muted">{o.hinweis}</span>}</span></Link>
                  : <div className="flex items-start gap-3 px-5 py-3 text-[13.5px]"><Circle size={17} className="text-amber shrink-0 mt-0.5" /><span className="flex-1">{x.text}</span></div>}
              </li>
            ); })}</ul>
          ) : <p className="text-[13.5px] px-5 pb-4">Alles erledigt – danke! Wenn noch etwas fehlt, melden wir uns im Chat.</p>}
        </section>
        ...
        {fertig > 0 && (
          <section className="card">
            ...
            <ul className="divide-y divide-line">{punkte.filter((o) => ob[o.key]).map((o) => (
              <li key={o.key} className="flex items-start gap-3 px-5 py-2.5 text-[13.5px] text-muted"><CheckCircle2 size={17} className="text-teal shrink-0 mt-0.5" /><span className="flex-1">{SELBST[o.key].text}</span></li>
            ))}</ul>
          </section>
        )}
      </div>
