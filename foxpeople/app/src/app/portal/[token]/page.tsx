...
export default async function PortalPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ danke?: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-bg">
      <header className="text-white px-6 py-4" style={{ background: "#10222a" }}><div className="flex items-center gap-3"><Logo size={32} /><span className="ml-auto text-white/60 text-[12px]">Mitarbeiter-Portal</span></div></header>
      <div className="max-w-2xl mx-auto p-6">
        {!gueltig ? (
          <div className="card card-pad text-center"><h1 className="font-display font-extrabold text-xl mb-2">Link ungültig oder abgelaufen</h1><p className="text-muted text-[13.5px]">Bitte bei Fox & People einen neuen Zugangslink anfordern.</p></div>
        ) : (
          <>
            <h1 className="font-display font-extrabold text-2xl mb-1">Hallo {t.person.vorname}!</h1>
            <p className="text-muted text-[13.5px] mb-5">Hier findest du deine Unterlagen. Dieser Link ist bis {datum(t.gultigBis)} gültig.</p>
            ...
            {t.person.einsaetze.length > 0 && (
              <div className="card card-pad mb-4">
                ...
                <form action={kundeBewertenPortal.bind(null, token)} className="space-y-3">
                  ...
                  <label className="flex items-center gap-2 text-[13.5px]"><input type="checkbox" name="wieder" defaultChecked /> Ich würde dort wieder arbeiten.</label>
            ...
            <div className="card">
              ...
              {t.person.dokumente.length ? <ul className="divide-y divide-line">{t.person.dokumente.map((d) => <li key={d.id} className="px-5 py-3 flex items-center gap-3"><div className="flex-1"><div className="font-semibold text-[13.5px]">{d.dateiname}</div><div className="text-[12px] text-muted">{d.kategorie} · {datum(d.hochgeladenAm)}</div></div><a href={`/portal/${token}/dokument/${d.id}`} className="btn btn-secondary btn-sm"><FileDown size={14} /> Öffnen</a></li>)}</ul> : <p className="px-5 pb-5 text-muted text-[13.5px]">Noch keine Dokumente freigegeben.</p>}
            </div>
            <p className="text-[12px] text-muted mt-6">Fragen? office@foxandpeople.at · +43 676 4574096 · Bitte diesen Link nicht weitergeben.</p>
          </>
        )}
      </div>
    </main>
